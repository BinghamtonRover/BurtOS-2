...
bool gui::Module::mouse_motion_event(const nanogui::Vector2i &p, const nanogui::Vector2i &rel, int button, int modifiers) {
	...
	if (m_resizable) {
		if (m_fixed_size.x() == 0 && check_horizontal_resize(p) != 0) {
			m_cursor = nanogui::Cursor::HResize;
		} else if ( m_fixed_size.y() == 0 && check_vertical_resize(p) != 0) {
...
bool gui::Module::mouse_button_event(const nanogui::Vector2i& pos, int button, bool down, int mods) {
    ...
    if (button == GLFW_MOUSE_BUTTON_1) {
        m_drag = down && (pos.y() - m_pos.y()) < m_theme->m_window_header_height;
        m_resize = false;
        if (m_resizable && !m_drag && down) {
            m_resize_dir.x() = (m_fixed_size.x() == 0) ? check_horizontal_resize(pos) : 0;
            m_resize_dir.y() = (m_fixed_size.y() == 0) ? check_vertical_resize(pos) : 0;
